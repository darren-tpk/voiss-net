...
location = ''
channel = '*DF'
pad = 360  # padding length [s] (must be greater and process_taper)
...
for i in range(num_steps):
    ...
    while not successfully_computed:
        try:
            ...
            tmpl, rmsp, pe, fc, fd, fsd = compute_metrics(stream, process_taper=60, metric_taper=pad,
    ...
    rmsp = correct_dimensions(rmsp, stream_stations)
    ...
    # Get PS4A trace
    if 'PS4A' in stream_stations:
        PS4A_trace = Stream(stream[stream_stations.index('PS4A')])
        try:
            PS4A_trace = process_waveform(PS4A_trace, remove_response=True, detrend=False, taper_length=pad,
                                      taper_percentage=None, filter_band=None, verbose=False)
        except:
            successfully_processed = False
            while not successfully_processed:
                try:
                    PS4A_trace = gather_waveforms(source=source, network=network, station='PS4A', location=location,
                                              channel=channel, starttime=t1 - pad, endtime=t2 + pad, verbose=False)
                    PS4A_trace = process_waveform(PS4A_trace, remove_response=True, detrend=False, taper_length=pad,
                                              taper_percentage=None, filter_band=None, verbose=False)
                    successfully_processed = True
                except:
                    print('gather_waveforms failed to retrieve response for PS4A')
                    pass

        # Calculate spectrogram
        PS4A_trace = PS4A_trace[0]
        spec_db, utc_times = calculate_spectrogram(PS4A_trace, t1, t2, window_duration, freq_lims, overlap=0)
    else:
    ...
    if i == 0:
        ...
        rmsp_all = np.hstack((rmsp_all, rmsp))
...
np.save(metrics_dir + 'rmsp_all' + tag + '.npy', rmsp_all)
