...
starttime = UTCDateTime(2023, 3, 22, 0, 0, 0)
...
if model_used == 'station-generic':
    spec_paths_full = glob.glob('/Users/darrentpk/Desktop/pavlof_test_npy/*.npy')
    spec_paths = []
    ...
    for station in stations:
        spec_paths_full = glob.glob('/Users/darrentpk/Desktop/pavlof_test_npy/*' + station + '*.npy')
        spec_paths = []
        for spec_path in spec_paths_full:
            utc = UTCDateTime(spec_path.split('/')[-1].split('_')[1])
            spec_station = spec_path.split('/')[-1].split('_')[0]
