...
json_filepath = '/Users/darrentpk/Desktop/GitHub/tremor_ml/labels/Semi_tremor_labels_20210701_20210831.json'
time_step = 4 * 60  # Create a training dataset with 2D matrices spanning 4 minutes each
output_dir = '/Users/darrentpk/Desktop/all_npys/labeled_npy_4min_semi/'
source = 'IRIS'
network = 'AV'
...
for labeled_image in labeled_images:
    ...
    for annotation in annotations:
        if len(annotation['value']['rectanglelabels']) != 0:
    ...
    for i, stream_station in enumerate(stream_stations):
        ...
        for j in range(len(step_bounds) - 1):
            ...
            # Enforce shape
            if np.shape(spec_slice) != (194, time_step):
            ...
            # If we see the non-tremor label in >10% of the time samples, choose non-tremor
            elif label_dict['Long Period'] in labels_seen and label_counts[list(labels_seen).index(label_dict['Long Period'])] >= 0.1 * len(label_indices):
                final_label = 'Long Period'
            # If we see the non-tremor label in >10% of the time samples, choose non-tremor
